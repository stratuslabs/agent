...
- **`GET /api/v1/catalog/tools`, inherited from [05](./05-control-api.md).** The control API shipped every endpoint of its management group except this one: with no plugins to list it could only report the three kernel tools, which would read as "these are your installed plugins" and be wrong. It needs a `gateway.tools()` accessor, which was deliberately not added there either. The dashboard's Plugins screen is a placeholder saying so, and the tool views in [17](./17-fleet-console.md) have nothing to bind to until this lands. (They were 07's when this was written; 07 has since been rewritten as a distribution and lifecycle step that holds no management screens.)
...
- `GET /api/v1/catalog/tools` lists the installed plugins with each tool's risk level, and the dashboard's Plugins screen renders it instead of its placeholder.
