...
- A wait that outlived its window while the process was down is denied rather than re-asked, measured from `parkedAt` against the configured timeout — the transport's original deadline was chosen after the checkpoint was written and died with the process that chose it. The queue behind it still drains.
