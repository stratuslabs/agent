...
- Aborting a turn while its approval is pending invalidates the request: the prompt is expired, and a subsequent Allow click is rejected instead of executing a tool for a cancelled turn (the abort signal reaches approval waits via `ApprovalContext` — see 01).
