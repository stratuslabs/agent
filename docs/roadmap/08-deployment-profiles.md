...
- A multi-tenant gateway runs ≥2 isolated tenants: no cross-tenant session/memory visibility (tested), per-tenant cost accounting matches provider-reported usage within rounding, a tenant hitting its cap is stopped cleanly with a friendly message.
