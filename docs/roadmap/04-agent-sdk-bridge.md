...
- **Attributing Slack renderers by turn id.** A renderer is queued at intake, *before* the gateway starts the turn it belongs to, so a message arriving while a recovery or the startup sweep is still ahead of it on the session chain leaves that turn's events looking rendered when they are not: events from a turn the adapter never dispatched go to whichever renderer heads the queue, and a `session.failed` from such a turn is suppressed rather than reported. [05](./05-control-api.md) landed the seam that fixes this — `dispatch` takes a `turnId` and the gateway exposes `activeTurnId(sessionId)` — but did not migrate the adapter onto it, so both consequences are still live.
