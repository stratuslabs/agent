...
- SQLite via `node:sqlite` (Node 22.5+, already our floor) to keep the zero-heavy-deps ethos; one `sessions` table (id, agent_id, status, JSON body, timestamps) is enough — no ORM.
