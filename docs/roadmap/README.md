...
| 24 | [Workers: shared utility agents the whole roster can call](./24-workers.md) | Not started — **Next** | An agent configured as a service rather than a teammate: no persona, no memory, no channel presence, invocable by delegation |
...
- **[24](./24-workers.md) — workers.** A roster has specialists as well as named teammates, and `createDelegateTool` already runs another agent in its own sub-session under that agent's allowlist and credentials — so this is mostly configuration rather than mechanism. The argument that pays is context offloading: a worker that reads five pages and returns three hundred words keeps fifty thousand tokens out of the calling agent's context, on a cheaper model, and model spend dominates everything else in a fleet. Names a vocabulary decision (**worker**, not "sub-agent", which means the opposite elsewhere) and the line that keeps it from eating *tool* and *skill*.
...
- **[07](./07-macos-app.md) — macOS app.** Dropped. Its stated job — creating and managing agents visually — is what [17](./17-fleet-console.md) delivers, sooner and on every platform, against an API that already exists. A second native surface for the same job is not worth what it costs to keep current. Nothing here forecloses picking it up again.
- **[12](./12-plugin-registry.md) — plugin registry.** Deferred, not dropped. Discovery and distribution for an ecosystem that does not exist yet is a platform built for nobody; the trigger to revisit is third-party plugins existing that we did not write. Note that [19](./19-registration-seams.md) is a genuine prerequisite either way — there is no point distributing channel or provider plugins that nothing can register.
