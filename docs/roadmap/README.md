...
| 04 | [Agent SDK tool bridge: tools + real history on the subscription path](./04-agent-sdk-bridge.md) | Partly shipped — tool bridge and dispatcher-side persistence done (#31, #51); tool bridge, SDK-native history, parity tests, and streaming deltas with the watchdog's tool-phase signal all shipped; clean restart on that path still open | Claude-subscription agents get full tool calling under kernel policy |
