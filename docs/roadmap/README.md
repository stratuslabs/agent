...
| 07 | [Desktop app: install to a running agent without a terminal](./07-desktop-app.md) | Not started — **Now**, and mostly unblocked | A signed macOS download that reaches a working agent in under two minutes, with no terminal at any point |
...
- **[07](./07-desktop-app.md) — desktop app.** Rewritten and un-parked. The
  step it used to be — a second native surface for managing agents — is still
  [17](./17-fleet-console.md)'s job and is still not worth duplicating. What it
  is now is the only answer to a narrower problem: every path into this product
  runs through a terminal, so 17 makes the management surface reachable for
  people who already installed a daemon, and nothing makes the daemon reachable
  at all. Its onboarding is a [16](./16-templates.md) template picker, which is
  why it sits here rather than above — but only that one screen is blocked, and
  the packaging, bootstrap, sign-in, and update work is not. The runtime
  questions are already settled in
