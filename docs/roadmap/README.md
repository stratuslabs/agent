...
| 25 | [Skills interop: conform to the standard, consume the ecosystem](./25-skills-interop.md) | Shipped — the conformance pass against the Agent Skills spec and its written list of differences ([`skill-format.md`](../reference/skill-format.md)); `name` as the id per the spec, required and directory-matched at install while loading stays tolerant of what is already installed; the Stratus extensions moved under `metadata:` (`requires`, `version`) with the top-level form still loading; strict validation on the shipped `stratus skill add` path — a non-conforming skill refused naming the problem, other hosts' fields and a bundled `scripts/` warned about, `compatibility` printed — and `stratus skill validate` running the same check without installing. Of the open questions: we differed in three places that mattered (`name` was a display string, the extensions were top-level keys, the id rule was looser than the spec's), all closed; a git URL is enough, no ecosystem-aware source form; publishing our own skills outward stays a distribution decision | A skill written for the open standard works here, and one written here works elsewhere |
| 27 | [Live reload: install a skill without restarting the fleet](./27-live-reload.md) | Shipped — `reloadSkills()` on the gateway (a whole-catalog rebuild into a fresh `SkillRegistry`, swapped in with `replaceWith`, strict on the operator directory so a broken file refuses the reload and the previous set keeps serving; plugin skills re-registered in load order so operator precedence and contested aliases come out as a restart would), `POST /skills/reload`, `stratus skill reload`, and `stratus skill add` reloading the daemon `gateway.json` names by default (`--no-reload`); the announced restart — `restart()` refuses new turns, drains for a window, aborts what outlives it with the watchdog's own signal, stops, and hands the process to the host, which `stratus serve` answers by starting a fresh daemon as a child and supervising it, so the same path holds under systemd, launchd, the foreground, and `--no-login`; `POST /restart` and `stratus restart --drain-timeout`; the what-needs-a-restart table in the always-on guide. Of the open questions: the drain window is bounded, forced by abort rather than exit, and per-request configurable; `skill add` reloads by default; disabling a plugin without a restart stays open | Skills reload live; plugins still restart, but announced and drained |
