...
| 04 | [Agent SDK tool bridge: tools + real history on the subscription path](./04-agent-sdk-bridge.md) | Partly shipped — tool bridge and dispatcher-side persistence done (#31, #51); SDK-native history, parity tests, the watchdog's view of a hosted approval, and clean restart on that path still open | Claude-subscription agents get full tool calling under kernel policy |
