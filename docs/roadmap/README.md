...
| 18 | [Usage accounting: `usage` on `ProviderResponse`](./18-usage-accounting.md) | Not started — **Now** | Per-turn token accounting the runner accumulates and emits — what every cost question and every budget cap needs first |
| 19 | [Registration seams: providers, channels, and memory stores as real plugins](./19-registration-seams.md) | Not started — **Next** | `PluginContext` grows past `{ bus, tools }` — four contribution kinds registered *by a plugin* instead of host-wired, with one plugin of each kind as the proof |
