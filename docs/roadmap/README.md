...
| 05 | [Control API + web dashboard](./05-control-api.md) | In progress — shared rules extracted from the CLI, gateway seams (turn ids, listable approvals, roster reload), and `@stratusagent/control-api` with auth, the management group, and the WS event stream; the web dashboard is the remaining piece | One API for every surface; a real chat/monitoring UI |
