...
12. `AgentDefinition.delegates?: string[]`, and `delegates` added to the soul frontmatter list keys — the allowlist shape `credentials` has, including its omitted-means-none default. `agent.delegate` takes its target from model input and is `safe`, so before this an agent holding the tool could run a turn as any agent on the roster, under that agent's tools, credentials, and memory. The kernel carries the field; the delegate tool in `agents` enforces it against the session's agent, the way the runner enforces `tools`.
