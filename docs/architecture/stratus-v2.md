...
**The mechanical cost is small, but it is not confined to the CLI.** `StratusProviderName` and `CredentialProviderName` are closed unions in `state`, and a third name has to reach every surface that hard-codes the pair instead of deriving it: the five `['anthropic', 'openai'] as const` sweeps and the two value predicates across `state` and `cli`, `defaultApiKeyEnvName`, the setup menu's provider list, `parseProviderParam` and the credentials response in `packages/control-api/src/routes.ts`, and `PROVIDERS` in `packages/dashboard/ui/views/settings.js`. Miss the last two and an implemented Codex provider is simply unreachable through the control API and the dashboard. The `verifyProviderKey` carve-out that already exists — a Claude subscription token cannot be checked against a models endpoint — extends to a ChatGPT one unchanged.

**One question is genuinely open, and it gets answered in this document rather than in an implementation PR**, because it can invalidate the approach rather than merely complicate it. **Which transport carries kernel tools into the Codex loop?** The Claude Agent SDK accepts an in-process MCP server, which is why `bridgeKernelTools` costs no transport and exposes no environment. Codex declares MCP servers in configuration as either a spawned command or a streamable HTTP URL (`codex mcp add --url`, with a bearer token read from a named variable), so the daemon can host a loopback endpoint rather than spawn anything — on that path the scrubbed-environment invariant is not what is at stake. What replaces it is a listening socket carrying every kernel tool call: what authenticates it, and what stops any other process on the machine from driving an agent's tools through it. If neither transport can carry them, Codex is a delegated *tool*, not a provider — a different design that should not reuse this name.

**One thing is not open: Stratus stays authoritative over tool execution.** Codex ships its own sandbox (`read-only` / `workspace-write` / `danger-full-access`) and approval policy (`untrusted` through `never`), and those govern model-generated shell commands *inside its harness* — commands that never traverse `ToolRegistry` → `ApprovalPolicy` → `Executor`. Letting the Codex policy decide is precisely the side door decision 1 rules out, so the only shippable configuration is its policy pinned to pass-through **and** its native shell and file tools disabled, leaving the kernel's chain the sole gate. If they cannot be disabled, this is the delegated-tool design above and not a provider.
