...
**A host that starts the gateway itself owns the state migration.** `stratus serve` claims the home (`claimHome`) and runs the pending migrations before it ever calls `createGateway`. A host doing its own wiring must do the same on a home written by a build before the per-agent layout: `start()` refuses one that still holds a shared `sessions.db` or an `<id>.whitelist.json`, rather than opening empty stores over it and starting a second population beside every session, schedule, and grant already there. The message names the fix — claim the home, then `await runStateMigrations(env, { exclusive: true })` from `@stratusagent/state`. The gateway will not do it unasked: the move needs the home to itself, and only the host knows whether it has it.
