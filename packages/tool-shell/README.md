...
| `cwd` | the agent's workspace | Where commands start. The default — `~/.stratus/agents/<agent-id>/workspace`, which the host answers for — is created on first use; a directory you name is not, and a missing one is reported by name rather than as a spawn failure. |
