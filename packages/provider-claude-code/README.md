...
- Images on the newest message (a screenshot sent in Slack) reach the model as images: a run whose message carries one is sent through the SDK's message-stream input with the images as `image` blocks beside the text, and a run without one keeps the plain prompt string. Older images in a replayed transcript are named, not re-sent. An image the API refuses to process is emptied on the session and the turn replays into a fresh SDK session with a note in its place — fresh, because the SDK already recorded the refused message in the one it was resuming.
