...
// argv parsing, the agent runtime, approval reach, and the helpers that talk
// to a running daemon. `bin.ts` imports this file, so anything here loads on
// every command: the gateway is imported lazily, by type only, for that
// reason.
...
export { menuPrefixWidth } from './prompter.ts';
...
export { runTemplateAdd } from './commands/template.ts';

export { runSkillAdd, runSkillValidate, runSkillReload, runSkills } from './commands/skills.ts';

export {
  type PluginToolReport,
  type PluginReport,
  type PluginsReport,
  collectPluginsReport,
  runPlugins,
} from './commands/plugins.ts';

export { runGrants } from './commands/grants.ts';
